...
function drop_preprocess_html(&$vars) {
  ...
  foreach ($user->roles as $role){
    $vars['classes_array'][] = 'role-' . drop_id_safe($role);
  }
  ...
  if (!$vars['is_front']) {
    ...
    $vars['classes_array'][] = drop_id_safe('page-'. $path);
    $vars['classes_array'][] = drop_id_safe('section-'. $section);
  ...
  // Add IE classes.
  if (theme_get_setting('drop_ie_enabled')) {
    $drop_ie_enabled_versions = theme_get_setting('drop_ie_enabled_versions');
    if (in_array('ie8', $drop_ie_enabled_versions, TRUE)) {
...
function drop_preprocess_page(&$vars, $hook) {
...
function drop_preprocess_node(&$vars) {
  ...
  // Merge first/last class (from drop_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
function drop_breadcrumb($variables) {
  ...
  $show_breadcrumb = theme_get_setting('drop_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('drop_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('drop_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('drop_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('drop_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 *
 */
function drop_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = drop_id_safe($element['#title']);
 ...
 */
function drop_preprocess_menu_local_task(&$variables) {
 ...
 */
function drop_menu_local_tasks(&$variables) {
  $output = '';
